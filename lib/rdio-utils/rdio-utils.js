...
(function(R, rdioUtils) {
  ...
  // ----------
  rdioUtils.TrackWidget = function(tracks) {
    var i;
    ...
    if (tracks) {
      if (tracks.key && !/^(t)[0-9]/.test(tracks.key)) {
        rdioUtils._error('[rdioUtils] Bad key for track widget: ' + tracks.key);
        this._broken = true;
      }

      var required = ['url', 'icon', 'name', 'artist', 'artistUrl', 'key'];
      for (i = 0; i < required.length; i++) {
        if (!tracks[required[i]]) {
          rdioUtils._error('[rdioUtils] Missing ' + required[i] + ' for track widget', tracks);
          this._broken = true;
        }
      }
    } else {
      rdioUtils._error('[rdioUtils] Track is required for track widget');
      this._broken = true;
    }

    if (this._broken) {
      this._element.innerHTML = ''
        + '<div class="rdio-utils-album-title rdio-utils-truncated">Unknown track</div>'
        + '<div class="rdio-utils-album-author rdio-utils-truncated">&nbsp;</div>'
        + '<div class="rdio-utils-album-size rdio-utils-truncated">&nbsp;</div>';
    ...
    var html = ''
          + '<a href="http://www.rdio.com' + rdioUtils._escape(tracks.url) + '">'
            + '<div class="rdio-utils-album-icon" style="background-image: url(' + rdioUtils._escape(tracks.icon) + ')"></div>'
            + '<div class="rdio-utils-album-hover-overlay">'
              + '<div class="rdio-utils-album-play-btn"></div>'
            + '</div>'
          + '</a>'
        + '<div class="rdio-utils-album-title rdio-utils-truncated"><a href="http://www.rdio.com' + rdioUtils._escape(tracks.url) + '">' + rdioUtils._escape(tracks.name) + '</a></div>'
        + '<div class="rdio-utils-album-author rdio-utils-truncated"><a href="http://www.rdio.com' + rdioUtils._escape(tracks.artistUrl) + '">' + rdioUtils._escape(tracks.artist) + '</a></div>';
  ...
  // ----------
  rdioUtils.TrackWidget.prototype = {
    // ----------
    element: function() {
      return this._element;
    },
